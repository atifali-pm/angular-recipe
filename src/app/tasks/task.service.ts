...
export class TaskService {
  ...
  getTasks(projectId: string) {
    return this.http.get<{ message: string, tasks: any }>('http://localhost:3000/api/projects/' + projectId + '/tasks')
      .subscribe(taskData => {
  ...
  getTask(id: string) {
    return this.http.get<{ _id: string, title: string, content: string, _projectId: string }>('http://localhost:3000/api/tasks/' + id);
  }
